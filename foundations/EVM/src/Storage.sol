...
// @ztmy
// how to assign variables using yul
// sstore
// sload
...
contract EVMStoragePackedSlotBytes {
    ...
    // @ztmy
    // the a in 0xabababab is a Hexadecimal character instead of a string letter
    // Variables less than 32 bytes are stored from the right (low position) (fill the left with 0)
    bytes4 public b4 = 0xabababab;
    bytes2 public b2 = 0xcdcd;
    // b4 and b2 stored in stroge(0) automatically
...
// @ztmy create bit masking
// shift
// shift the shifted
// invert all the zeros and ones 
contract BitMasking {
    function test_mask() public pure returns (bytes32 mask) {
        assembly {
            @ztmy
            // shift is operate on bits
            // |       256 bits        |
            // 000 ... 001 | 000 ... 000
            // sub 1 =
            // 000 ... 000 | 111 ... 111
...
//@ztmy
// use these bit masking tricks to read and write from State variable that are packed into a single slot
