...
@view
@internal
# @ztmy convert the token balances inside this contract to have 18 decimals
def _xp() -> uint256[N_COINS]:
    result: uint256[N_COINS] = RATES # [1e18, 1e30, 1e30]
    for i in range(N_COINS):
...
@view
@external
# Returns the value of each LP token
...
@view
@external
# Returns the amount of LP tokens to be minted or to be burnt
def calc_token_amount(amounts: uint256[N_COINS], deposit: bool) -> uint256:
    ...
    for i in range(N_COINS):
        # @ztmy
        #  if it is a deposit then add the amounts, 
        #  else is a withdrawal, is a remove liquidity, then take out amounts
        if deposit:
    ...
    diff: uint256 = 0
    # if we're adding tokens liquidity must have increased
    if deposit:
        diff = D1 - D0
    else:
        diff = D0 - D1
    # diff divided by d0 represents the ratio of change in liquidity 
    # and then multiply by token amount to calculate the amount of LP tokens
...
def add_liquidity(amounts: uint256[N_COINS], min_mint_amount: uint256):
    ...
    #@ztmy _fee is used to charge for use add/remove liquidity to do swap
    # x -> swap -> y (swap fee token y)
    # x -> add liq
    #           remove liq -> y
...
def get_y(i: int128, j: int128, x: uint256, xp_: uint256[N_COINS]) -> uint256:
    ...
    assert i < N_COINS
    # calculate A
    # calculate D : because the liquidity has changed
    amp: uint256 = self._A()
...
@view
@external
#@ztmy used for user. Return Real token unit.
...
# ---------------------------------------calc y ,dy_fee, dy--------------------------------------------------------------------------------
    ...
    # @ztmy convert this normalized balance back into the real balance
...
# ---------------------------------------update balances and transfer out--------------------------------------------------------------------------------
    ...
    #@ztmy dy_fee-dy_admin_fee remains in the pool as a reward for the LP provider
...
### Admin functions ###
# @ztmy these are functions that the admin calls to set the A parameter
@external
