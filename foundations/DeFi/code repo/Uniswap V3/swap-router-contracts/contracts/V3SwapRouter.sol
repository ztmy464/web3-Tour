...
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage {
    ...
    function exactInputInternal(
        ...
        // @ztmy V3 support use Constants.MSG_SENDER to flag recipient to save gas
        if (recipient == Constants.MSG_SENDER) recipient = msg.sender;
        else if (recipient == Constants.ADDRESS_THIS) recipient = address(this);
        //@ztmy [DAI, 100, USDC, 3000, WETH]
        (address tokenIn, address tokenOut, uint24 fee) = data.path.decodeFirstPool();
        ...
        returns (uint256 amountOut)
    {
        ...
        //@ztmy hasAlreadyPaid == false: token in will from the caller.
        bool hasAlreadyPaid;
    ...
    function exactOutputInternal(
        ...
        // @ztmy The recipient for the next swap is set to the previous pool contract.
        if (recipient == Constants.MSG_SENDER) recipient = msg.sender;
        else if (recipient == Constants.ADDRESS_THIS) recipient = address(this);
        // DAI -> USDC -> WETH
