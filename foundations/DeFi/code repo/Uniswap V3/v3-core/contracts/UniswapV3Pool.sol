...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    function swap(
        address recipient,
        bool zeroForOne,
        // @ztmy When this number is a positive number, then means that this swap is an exact input.
        int256 amountSpecified,
        // This is the price limit that the caller is willing to accept.
        ...
        // @ztmy
        // Ensure that the liquidity pool is not currently locked
        ...
        // @ztmy
        // Lock the pool at the start of the transaction
        slot0.unlocked = false;
        ...
        // @ztmy
        // This swap state will be used inside a while loop to execute swaps.
        ...
        // @ztmy
        // this will calculate the final amount of token that must come in and go out
        // It will also update the current tick and the current square root price X96 stored in memory.
        while (state.amountSpecifiedRemaining != 0 && state.sqrtPriceX96 != sqrtPriceLimitX96) {
            StepComputations memory step;

            step.sqrtPriceStartX96 = state.sqrtPriceX96;

            // first get the next tick.
...
// ---------------------------------------Process an swap for a price range--------------------------------------------------------------------------------
...
// exactInput == true（输入金额确定，计算可得金额）：

// state.amountSpecifiedRemaining 减少 step.amountIn + step.feeAmount
// → 代表已经消耗的输入代币数量（包括手续费）。
// state.amountCalculated 减少 step.amountOut
// → 代表已经获得的输出代币数量。
...
// ---------------------------------------update feeGrowthGlobalX128--------------------------------------------------------------------------------
        ...
        // These are state variable that track the amount of swap fees that was collected.
        // Using these state variables, the liquidity providers will be able to collect fees.
        if (zeroForOne) {
        ...
        // @ztmy
        // | zeroForOne| exactInput| amount0                       | amount1                       |
        // |-----------|-----------|-------------------------------|-------------------------------|
        // | true      | true      | specified - remaining (>0)    | calculated (<0)               |
        // | true      | false     | calculated (<0)               | specified - remaining (>0)    |
        // | false     | true      | calculated (<0)               | specified - remaining (>0)    |
        // | false     | false     | specified - remaining (>0)    | calculated (<0)               |
