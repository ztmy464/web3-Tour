...
library TickBitmap {
    ...
    // NOTE: flip a tick
    // tickSpacing: when we do a swap the tick does not necessarily update By One 
    // The Tick actually updates by tick spacing
    ...
    // NOTE: How to get Next tick
    // bool lte: the direction 
    ...
    function nextInitializedTickWithinOneWord(
        ...
        if (lte) {
            ...
            // if mask is equal to zero then there is no next tick on the left(initialized = 0)
            initialized = masked != 0;

            // nect = (compressed - remove bit pos + right most bit of masked) * tick spacing
            //      =  tick       - bit pos        + next bit pos
            next = initialized
